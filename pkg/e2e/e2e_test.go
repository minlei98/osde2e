...
func TestBuildNotificationConfig_Disabled(t *testing.T) {
	...
	orch := &E2EOrchestrator{}
	cfg := orch.buildNotificationConfig()
...
func TestBuildNotificationConfig_MissingCredentials(t *testing.T) {
	setupTestConfig(t)
	viper.Set(config.Tests.EnableSlackNotify, true)
	viper.Set(config.LogAnalysis.SlackWebhook, "")
	viper.Set(config.LogAnalysis.SlackChannel, "")
...
func TestBuildNotificationConfig_Enabled(t *testing.T) {
	...
	viper.Set(config.Tests.EnableSlackNotify, true)
	viper.Set(config.LogAnalysis.SlackWebhook, "https://hooks.slack.com/test")
	viper.Set(config.LogAnalysis.SlackChannel, "#test-channel")
...
func TestCollectLogs_NoProvider(t *testing.T) {
	setupTestConfig(t)
...
func TestCollectLogs_EmptyClusterID(t *testing.T) {
	setupTestConfig(t)
	viper.Set(config.Cluster.ID, "")

	orch := &E2EOrchestrator{
		provider: nil, // Don't need actual provider for this test
	}
