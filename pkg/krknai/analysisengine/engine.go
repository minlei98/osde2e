...
func (e *Engine) Run(ctx context.Context) (*analysisengine.Result, error) {
	...
	if e.config.ReportFormat == "html" {
		mustGatherPath := mustGatherRelativePath(e.config.ArtifactsDir)
		var err error
		content, err = markdownToHTML(content, mustGatherPath)
		if err != nil {
...
func markdownToHTML(content, mustGatherPath string) (string, error) {
	...
	}{Body: template.HTML(string(safeBody)), MustGatherPath: mustGatherPath}
